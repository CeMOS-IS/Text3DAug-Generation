...
def get_and_load_weights():
    ...
    weight_dir = os.path.join(THIS_DIR, ".gpt4point_weights")
    os.makedirs(weight_dir, exist_ok=True)

    # Actually download the weights
    weight_fn = os.path.join(weight_dir, "pyramidxl_point_e.pt")
    if not os.path.isfile(weight_fn):
        dl_url = "https://huggingface.co/alexzyqi/GPT4Point/resolve/main/gpt4point_diffusion_v1.pth?download=true"
        urllib.request.urlretrieve(dl_url, weight_fn)
        print("PyramidXL Point-E weights complete")
...
class BaseModel(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        """
        Class made for weight name compatibility.
        """

        # Text -> pointcloud model
        base_name = "base40M-textvec"
        self.point_e_model = model_from_config(MODEL_CONFIGS[base_name], DEVICE)
        self.point_e_model.eval()

        self.point_e_diffusion = diffusion_from_config(DIFFUSION_CONFIGS[base_name])

        # Upsampling model
        self.point_e_upsampler_model = model_from_config(
            MODEL_CONFIGS["upsample"], DEVICE
        )
        self.point_e_upsampler_model.eval()
        self.point_e_upsampler_diffusion = diffusion_from_config(
            DIFFUSION_CONFIGS["upsample"]
        )


class PyramidXLPointE:
    def __init__(self) -> None:
        # Meshing model
        name = "sdf"
        ...
        self.point_e_model = BaseModel()
        self.point_e_model.load_state_dict(get_and_load_weights(), strict=False)

        # Assemble models for sampling
        self.sampler = PointCloudSampler(
            device=DEVICE,
            models=[
                self.point_e_model.point_e_model,
                self.point_e_model.point_e_upsampler_model,
            ],
            diffusions=[
                self.point_e_model.point_e_diffusion,
                self.point_e_model.point_e_upsampler_diffusion,
            ],
    ...
    def __call__(self, prompt: str, debug: bool = False) -> o3d.geometry.TriangleMesh:
        """Generates a mesh from a text prompt using GPT4Point and marching cubes.
    ...
    def preprocess(self, prompt):
        """
        Put the prompt in the proper format for GPT4Point.
        """
        return dict(texts=[prompt])

    def generate(self, prompt):
        """
        Use GPT4Point to generate a pointcloud from text.
        """
    ...
    def generate_exception(self, rider: str, bike: str):
        """
        Handles exception classes not supported by GPT4Point.
        """
