...
def preprocess(predictor, raw_im, lower_contrast=False):
    ...
    input_256 = image_preprocess_nosave(
        image_sam, lower_contrast=lower_contrast, rescale=True
    )
...
def stage1_run(model, device, exp_dir, input_im, scale, ddim_steps):
    ...
    zero123_infer(
        model, exp_dir, indices=[0], device=device, ddim_steps=stage2_steps, scale=scale
    )
...
def stage2_run(model, device, exp_dir, elev, scale, stage2_steps=50):
    ...
    if 90 - elev <= 75:
        ...
        zero123_infer(
            model,
            exp_dir,
            indices=list(range(1, 4)) + list(range(8, 12)),
            device=device,
            ddim_steps=stage2_steps,
            scale=scale,
        )
...
def predict_multiview(shape_dir, args):
    ...
    elev, stage1_imgs = stage1_run(
        model_zero123, device, shape_dir, input_256, scale=3, ddim_steps=75
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img_path",
        type=str,
        default="./demo/demo_examples/01_wild_hydrant.png",
        help="Path to the input image",
    )
    parser.add_argument("--gpu_idx", type=int, default=0, help="GPU index")
    parser.add_argument(
        "--half_precision", action="store_true", help="Use half precision"
    )
    parser.add_argument(
        "--mesh_resolution", type=int, default=256, help="Mesh resolution"
    )
    parser.add_argument(
        "--output_format",
        type=str,
        default=".ply",
        help="Output format: .ply, .obj, .glb",
    )
    parser.add_argument("--folder", type=str, help="Folder to save in")
    ...
    shape_dir = f"./exp/{args.folder}/{shape_id}"
